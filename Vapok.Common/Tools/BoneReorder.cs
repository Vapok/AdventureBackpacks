...
namespace Vapok.Common.Tools
{
    ...
    public static class BoneReorder
    {
        ...
        /// </summary>
        public static void ApplyOnEquipmentChanged(string pluginId)
        {
            if (!applied)
            {
                _log = LogManager.GetLogger(pluginId);
                var harmony = new Harmony(pluginId);
                harmony.PatchAll(typeof(BoneReorder));
                applied = true;
            }
        }
        ...
        private static void ReorderBones(VisEquipment visEquipment, int itemPrefabHash, List<GameObject> instancesToFix)
        {
            if (visEquipment != null)
            {
                try
                {
                    ...
                    {
                        _log?.Debug($"Prefab missing components. Skipping {itemPrefab} {skeletonRoot}");
                        return;
                    }
                    _log?.Debug($"Reordering bones");
                ...
                {
                    _log?.Warning($"Exception caught while reordering bones: {ex}");
                }
            }
        }
